...
Public Class frmClearingClient
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If MessageBox.Show("Are you sure you want to continue?", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = vbYes Then
            For Each itm As ListViewItem In ListView1.Items
                If itm.Checked Then
                    Dim getBranch As String = qrysingledata("branch", "(select branchname from master.branches where branchcode = master.client.branchcode) as 'branch'", "master.client  where custcode = '" & itm.SubItems(1).Text & "'")
                    com.CommandText = "update master.client set trnlocked=false,lockedby='',lockdesc='' where custcode  like '" & itm.SubItems(1).Text & "' limit 1;" : com.ExecuteNonQuery()
            ...
            Next
            loadLoggedaccounts()
            MsgBox("Selected client successfully cleared!", MsgBoxStyle.Information)
        End If
    End Sub
    ...
    Public Sub loadLoggedaccounts()
        ...
        com.CommandText = "select * from master.client  where trnlocked=1 and (concat(lname, ', ',fname) like '%" & txtSearch.Text & "%' or custcode like '%" & txtSearch.Text & "%') order by concat(lname, ', ',fname) asc" : rst = com.ExecuteReader
        While rst.Read
            Dim item1 As New ListViewItem("(" & rst("custcode").ToString & ") - " & UCase(rst("lname").ToString) & ", " & UCase(rst("fname").ToString), 0)
            item1.SubItems.Add(rst("custcode").ToString)
