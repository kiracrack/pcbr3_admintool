...
Public Class frmDormantSettings
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If MessageBox.Show("Are you sure you want to continue?", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = vbYes Then
            For Each i As Object In ls.CheckedItems
                Dim getDepcode As String = qrysingledata("depcode", "depcode", "master.depositprod  where concat('(',depcode,') - ',lcase(description)) = '" & i.ToString & "'")
                If countqry("`action_query`.`tbldormantsettings`", "depcode='" & getDepcode & "'") > 0 Then
                    com.CommandText = "delete from action_query.tbldormantsettings where depcode='" & getDepcode & "'" : com.ExecuteNonQuery()
                End If
                com.CommandText = "INSERT into action_query.tbldormantsettings set depcode='" & getDepcode & "', term='" & txtTerm.Text & "'" : com.ExecuteNonQuery()
            Next
            LoadSavingAccount()
            LoadDormantSettings()
    ...
    Public Sub LoadDormantSettings()
        dst = New DataSet
        msda = New MySqlDataAdapter("select '' as No, depcode as 'Code', (select description from master.depositprod where depcode = tbldormantsettings.depcode) as 'Deposit Product',Term from action_query.tbldormantsettings order by (select description from master.depositprod where depcode = tbldormantsettings.depcode) asc ", conn)
    ...
    Private Sub ViewTransactionDetailsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ViewTransactionDetailsToolStripMenuItem.Click
        If MessageBox.Show("Are you sure you want to continue? ", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = vbYes Then
            For Each rw As DataGridViewRow In MyDataGridView.SelectedRows
                com.CommandText = "delete from action_query.tbldormantsettings where depcode='" & rw.Cells("Code").Value.ToString & "'" : com.ExecuteNonQuery()
            Next
            LoadDormantSettings()
        End If
    End Sub
